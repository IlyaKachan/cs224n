...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow.
    # the predicted distribution aka softmax
    ...
    gradCenterVec = np.matmul((yHat - yTrue), outsideVectors)
    gradOutsideVecs = np.outer((yHat - yTrue), centerWordVec)
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    negSampleWordVectors = outsideVectors[negSampleWordIndices]

    outsideWordScore = np.dot(outsideWordVec, centerWordVec)
    negSampleWordScores = np.matmul(negSampleWordVectors, centerWordVec)

    loss = (
        - np.log(sigmoid(outsideWordScore))
        - np.log(sigmoid(-negSampleWordScores)).sum()
    )

    outsideWordSigmoidScore = -sigmoid(-outsideWordScore)
    negSampleWordSigmoidScores = sigmoid(negSampleWordScores)

    gradCenterVec = (
        outsideWordSigmoidScore * outsideWordVec +
        np.matmul(negSampleWordSigmoidScores, negSampleWordVectors)
    )

    outsideMultiplier = np.zeros(outsideVectors.shape[0])
    outsideMultiplier[outsideWordIdx] = outsideWordSigmoidScore
    for neg_i, i in enumerate(negSampleWordIndices):
        outsideMultiplier[i] += negSampleWordSigmoidScores[neg_i]

    gradOutsideVecs = np.outer(outsideMultiplier, centerWordVec)
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        ...
        loss += currLoss
        gradCenterVecs[currentCenterWordIdx] += currGradCenterVec
