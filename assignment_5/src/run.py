...
if args.function == 'pretrain':
    ...
    t = trainer.Trainer(gpt, pretrain_dataset, None, tconf)
    t.train()
    t.save_checkpoint()
    ...
    finetune_dataset = dataset.NameDataset(
        pretrain_dataset, open(args.finetune_corpus_path).read())
    ...
    t = trainer.Trainer(gpt, finetune_dataset, None, tconf)
    t.train()
    t.save_checkpoint()
    ...
    gpt.load_state_dict(torch.load(args.reading_params_path, map_location=device))
    correct = 0
    total = 0
