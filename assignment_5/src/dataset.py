...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        trunc_document = document[:length]

        trunc_len = len(trunc_document)
        masked_length = int(trunc_len * random.uniform(1/8, 3/8)) or 1
        masked_pos = random.randint(1, trunc_len - 1 - masked_length)

        prefix = trunc_document[:masked_pos]
        masked_content = trunc_document[masked_pos: masked_pos + masked_length]
        suffix = trunc_document[masked_pos + masked_length:]
