...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        weights = torch.empty(self.n_features * self.embed_size, self.hidden_size)
        nn.init.xavier_uniform_(weights)
        self.embed_to_hidden_weight = nn.Parameter(weights)

        biases = torch.empty(self.hidden_size)
        nn.init.uniform_(biases)
        self.embed_to_hidden_bias = nn.Parameter(biases)

        self.relu = nn.ReLU()
        self.dropout = nn.Dropout(p=self.dropout_prob)

        weights = torch.empty(self.hidden_size, self.n_classes)
        nn.init.xavier_uniform_(weights)
        self.hidden_to_logits_weight = nn.Parameter(weights)

        biases = torch.empty(self.n_classes)
        nn.init.uniform_(biases)
        self.hidden_to_logits_bias = nn.Parameter(biases)
        ### END YOUR CODE
    ...
    def embedding_lookup(self, w):
        ...
        idx = torch.flatten(w)
        x_flat = torch.index_select(self.embeddings, dim=0, index=idx)
        x = x_flat.view((w.shape[0], self.n_features * self.embed_size))
        ### END YOUR CODE
        return x
    ...
    def forward(self, w):
        ...
        logits = h_drop.matmul(self.hidden_to_logits_weight) + self.hidden_to_logits_bias
        ### END YOUR CODE
        return logits
