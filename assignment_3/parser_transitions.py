...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == 'S':
            ...
            raise ValueError(f'Unaccepted transition value "{transition}"')
        ### END YOUR CODE
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        for t_idx, transition in enumerate(reversed(transitions)):
            i = len(transitions) - 1 - t_idx
            unfinished_parses[i].parse_step(transition)
            if unfinished_parses[i].is_complete:
                _ = unfinished_parses.pop(i)
